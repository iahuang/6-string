...
function chordDifficulty(chord: string) {
...
export function autoTransposeChords(chordsList: string[], range: number[] = [-12, 12]) {
    ...
    let calcAvgDifficulty = (steps: number) =>
        _.meanBy(chordsList, (chord) =>
            chordDifficulty(transpose(chord, steps))
        );
...
export function transpose(chord: string, steps: number): string {
    ...
    let transposeChart =
        mode === "flat" ? transposeChartFlat : transposeChartSharp;
    ...
    let transposedChordBase =
        transposeChart[mod(startingPos + steps, transposeChart.length)];
